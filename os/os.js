...
(function () {
  ...
  const Data = {
    // ----- Creators -----
    listCreators(){
      return SB.from('creators').select('*').eq('studio_id', studioId()).order('created_at', { ascending: false });
    },
    addCreator(p){
      return SB.from('creators').insert(Object.assign({ studio_id: studioId() }, p)).select().single();
    },
    updateCreator(id, p){
      return SB.from('creators').update(p).eq('id', id).select().single();
    },
    deleteCreator(id){
      return SB.from('creators').delete().eq('id', id);
    },
    ...
    listDeals(){
      return SB.from('deals')
        .select('*, creator:creators(id,name), client:clients(id,name)')
        .eq('studio_id', studioId())
        .order('created_at', { ascending: false });
    },
    addDeal(p){
      return SB.from('deals').insert(Object.assign({ studio_id: studioId() }, p)).select().single();
    },
    updateDeal(id, p){
      return SB.from('deals').update(p).eq('id', id);
    },
    moveDeal(id, stage){
      return SB.from('deals').update({ stage: stage }).eq('id', id);
    },
    ...
    async approveApplication(app){
      ...
      const ins = await SB.from('creators').insert({
        studio_id: studioId(),
        name: app.name,
        handle: app.handle,
        email: app.email,
      ...
      const upd = await SB.from('applications')
    ...
    addInvoice(p){
      return SB.from('invoices').insert(Object.assign({ studio_id: studioId() }, p)).select().single();
    },
    updateInvoice(id, p){
      return SB.from('invoices').update(p).eq('id', id);
    },

    // ----- Payouts -----
    listPayouts(){
      return SB.from('payouts')
        .select('*, creator:creators(id,name), deal:deals(id,title)')
        .eq('studio_id', studioId())
        .order('created_at', { ascending: false });
    },
    addPayout(p){
    ...
    async dashboardStats(){
      ...
      const [active, apps, paid, unread] = await Promise.all([
        SB.from('deals').select('id', { count: 'exact', head: true }).eq('studio_id', sId).neq('stage', 'paid'),
        SB.from('applications').select('id', { count: 'exact', head: true }).eq('status', 'new'),
        SB.from('invoices').select('amount_cents').eq('studio_id', sId).eq('status','paid')
  ...
  async function viewApplication(a){
    ...
    if (a.status === 'new' && SESSION) {
      const upd = await SB.from('applications').update({
        status: 'review',
          ...
          approveBtn.addEventListener('click', async function(){
            ...
            try {
              if (notes !== (a.reviewer_notes || '')) {
                await SB.from('applications').update({ reviewer_notes: notes }).eq('id', a.id);
              }
              await Data.approveApplication(a);
              await Data.logActivity('application.approved', 'Approved ' + a.name);
              toast('Added to roster.', 'success');
